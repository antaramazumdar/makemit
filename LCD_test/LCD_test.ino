...
void punish(){
  ...
  if (currentMillis - lastLCDUpdate >= lcdInterval) {
    ...
    lcd.write(byte(0));
    lcd.write(byte(0));
    // delay(200); --> we want the rest of the program to continue, but this should wait 200 ms before updating
  }
  int zeroedCount = i % (255 * 3);
  int loop = zeroedCount / 255;
  int loopValue = zeroedCount % 255;

  analogWrite(RED, (loop==0)*(loopValue) + (loop==1)*(255) + (loop==2)*(255-loopValue));
  analogWrite(GREEN, (loop==0)*(255-loopValue)  + (loop==1)*(loopValue) + (loop==2)*(255));
  analogWrite(BLUE,  (loop==0)*(255) + (loop==1)*(255-loopValue) + (loop==2)*(loopValue));
